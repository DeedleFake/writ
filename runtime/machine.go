...
// TypeBlobs, when set, holds encoded type descriptors (from types.Encode)
// keyed by export name. The host checker uses them instead of the defaults
// (funcs as (dynamic any), macros as macro, values as any). Authors normally
// set blobs via writ.WithPackageTypes / writ.ExportGuestPackage.
type Package struct {
...
func (m *Machine) Expand(forms []syntax.Form) (ir.Program, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.BeginBudget()
...
func (m *Machine) ExpandLocked(forms []syntax.Form) (ir.Program, error) {
	return compileForms(forms, m, false)
}
...
func (m *Machine) EvalLocked(forms []syntax.Form) (Value, error) {
	...
	m.handlers = append(m.handlers, bound...)
	if err != nil {
...
func (m *Machine) EvalModule(path string, forms []syntax.Form) (Value, error) {
	...
	m.rememberLoaded(path, &loadedPkg{exports: exp, handlers: bound, env: env})
	m.handlers = append(m.handlers, bound...)
	return exp, nil
}
...
func evalNamedImports(imps []ir.NamedImport, env *env, c *ctx) error {
