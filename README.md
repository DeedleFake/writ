...
Quote (`'`), unquote (`,`), splice (`@`), and comments are syntax forms, not runtime values. After eval they are gone, except nested quote which becomes a `syntax` value holding the remaining quote form (printed like `'x`).
...
Optional export types: use `writ.WithPackageTypes` / `writ.ExportGuestPackage` with a `map[string]types.Type`. Wire blobs live on `Package.TypeBlobs` so `runtime` never imports `types`. Typed package-table entries carry those descriptors so `writ check` / inference use them instead of `any` / `(dynamic any)` / `macro`. Untyped exports keep the old defaults.
