...
- `check` does not open native plugins (embed tests cover that). A `.so` import during check is not a CLI plugin-load proof. `check` does instantiate `.wasm` imports so macros can expand.
